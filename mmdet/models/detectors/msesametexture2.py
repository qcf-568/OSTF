...
class MSESameTextureDetector2(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        img_sames = torch.where(batch_data_samples[0].gt_instances.tamper_same<=1, batch_data_samples[0].gt_instances.tamper_same, 0)
        ...
        x = self.extract_feat(batch_inputs)
        x2 = [self.roi_head.convert1(xx[:,128:]) for xx in x]
        x = [self.roi_head.convert1(xx[:,:128]) for xx in x]
        
        if flag:
            mask_rois = mask_target(same_boxes, same_index, [batch_data_samples[0].gt_instances.masks], self.same_cfg).unsqueeze(1)
            auth_roi0 = self.msroialign([xx[0:1] for xx in x], same_boxes, img_shape)
            auth_roi1 = self.msroialign([xx[1:2] for xx in x], same_boxes, img_shape)
            tamp_roi0 = self.msroialign([xx[0:1] for xx in x2], same_boxes, img_shape)
            tamp_roi1 = self.msroialign([xx[1:2] for xx in x2], same_boxes, img_shape)
            ...
            auth_roi0_feat = F.normalize((auth_roi0 * mask_rois).sum((2,3))/mask_rois_sum)
            auth_roi1_feat = F.normalize((auth_roi1 * mask_rois).sum((2,3))/mask_rois_sum)
            tamp_roi0_feat = F.normalize((tamp_roi0 * mask_rois).sum((2,3))/mask_rois_sum)
            tamp_roi1_feat = F.normalize((tamp_roi1 * mask_rois).sum((2,3))/mask_rois_sum)
            same_label0 = torch.ones_like(same_label)
            auth_mse = ((auth_roi0_feat - auth_roi1_feat)**2).mean()
            tamp_mse = (((tamp_roi0_feat - tamp_roi1_feat)**2).mean(1) * (torch.where(same_label==0,1,-1))).mean()
            # auth_cos = torch.abs(self.cos(auth_roi0_feat, auth_roi1_feat))
            # tamp_cos = torch.abs(self.cos(tamp_roi0_feat, tamp_roi1_feat))
            # print('shape1', auth_cos.shape, same_label0.shape, tamp_cos.shape, same_label.shape)
            # auth_bce = self.bce(auth_cos, same_label0)
            # tamp_bce = self.bce(tamp_cos, 1-same_label)
            # print(mask_rois.shape, mask_rois.min(), mask_rois.max(), auth_roi0.shape, tamp_roi1.shape)
        # cos_similarity? constrative learning? GRL?
        # print(ms_roi0.shape, ms_roi1.shape)
        if flag:
            losses = dict(auth_mse=auth_mse*1024, tamp_mse=tamp_mse*1024)
        else:
            losses = dict()
        ...
        # exit(0)
        roi_losses = self.roi_head.loss(x, x2, rpn_results_list,
                                        batch_data_samples)
