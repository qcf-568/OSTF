...
class SameTextureDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if len(same_boxes)!=0:
            same_label = torch.LongTensor([b for b in img_sames if b!=-1], device=device)
            same_boxes = [torch.stack(same_boxes).to(device)]
            same_index = [torch.LongTensor([bi for bi in range(len(img_boxes)) if img_sames[bi]!=-1], device=device)] #, device=same_boxes.device)
            flag = True
        else:
            flag = False
        ...
        if flag:
            ...
            auth_roi0 = feat_roi0[:, :128]
            auth_roi1 = feat_roi1[:, :128]
            tamp_roi0 = feat_roi0[:, 128:]
            tamp_roi1 = feat_roi1[:, 128:]
            mask_rois_flat = mask_rois.flatten()
            mask_rois_max = mask_rois_flat.max(1).values
            mask_rois_sum = mask_rois_flat.sum(1)
            auth_roi0_feat = (auth_roi0 * mask_rois).sum((2,3))/mask_rois_sum
            auth_roi1_feat = (auth_roi1 * mask_rois).sum((2,3))/mask_rois_sum
            tamp_roi0_feat = (tamp_roi0 * mask_rois).sum((2,3))/mask_rois_sum
            tamp_roi1_feat = (tamp_roi1 * mask_rois).sum((2,3))/mask_rois_sum
            same_label0 = torch.zeros_like(same_label)
