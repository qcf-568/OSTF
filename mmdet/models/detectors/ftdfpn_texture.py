...
class FTDFPNTextureDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        if not (init_cfg is None):
            self.pretrained = init_cfg['pretrained']
            self.ptflag = True
        else:
            self.ptflag = False
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # exit(0)
        if self.ptflag:
            self.load_state_dict(torch.load(self.pretrained, map_location='cpu')['state_dict'])
            self.ptflag = False
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        for i in range(len(batch_data_samples)):
            # print(hasattr(batch_data_samples[i].gt_instances, 'labels'), hasattr(batch_data_samples[i].gt_instances, 'tamper_type0'))
            batch_data_samples[i].gt_instances.tamper = batch_data_samples[i].gt_instances.labels
            batch_data_samples[i].gt_instances.labels = torch.zeros_like(batch_data_samples[i].gt_instances.labels)
        ...
        if self.with_rpn:
            ...
            rpn_losses, rpn_results_list = self.rpn_head.loss_and_predict(
                x1, rpn_data_samples, proposal_cfg=proposal_cfg)
        ...
        # print(batch_data_samples[0].img_path+'|'+batch_data_samples[1].img_path)
        roi_losses = self.roi_head.loss(x1, x2, rpn_results_list,
                                        batch_data_samples)
        # print('texture.py roi_loss', roi_losses)
        losses.update(roi_losses)
        # print(batch_data_samples[0].img_path+'|'+batch_data_samples[1].img_path+str(losses))
        return losses
    ...
    def predict(self,
        ...
        results_list = self.roi_head.predict(
            x1, x2, rpn_results_list, batch_data_samples, rescale=rescale)
        return results_list
