...
class GLBSameTextureDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        img_boxes = [bds.gt_instances.bboxes for bds in batch_data_samples]
        img_tamps = [bds.gt_instances.tamper for bds in batch_data_samples]
        valid_boxes0 = [b for bi,b in enumerate(batch_data_samples[0].gt_instances.bboxes) if (batch_data_samples[0].gt_instances.tamper[bi]!=-1)]
        valid_boxes1 = [b for bi,b in enumerate(batch_data_samples[1].gt_instances.bboxes) if (batch_data_samples[1].gt_instances.tamper[bi]!=-1)]
        valid_tamper0 = [b for bi,b in enumerate(batch_data_samples[0].gt_instances.tamper) if (batch_data_samples[0].gt_instances.tamper[bi]!=-1)]
        valid_tamper1 = [b for bi,b in enumerate(batch_data_samples[1].gt_instances.tamper) if (batch_data_samples[1].gt_instances.tamper[bi]!=-1)]
        valid_index0 = torch.cuda.LongTensor([bi for bi,b in enumerate(batch_data_samples[0].gt_instances.bboxes) if (batch_data_samples[0].gt_instances.tamper[bi]!=-1)], device=device)
        valid_index1 = torch.cuda.LongTensor([bi for bi,b in enumerate(batch_data_samples[1].gt_instances.bboxes) if (batch_data_samples[1].gt_instances.tamper[bi]!=-1)], device=device)
        x = self.extract_feat(batch_inputs)
        if ((len(img_boxes[0])!=0) and (len(valid_boxes0)!=0)):
            flag0 = True
            h,w =  x[3].shape[-2:]
            msk0 = torch.ones((1, 1, h, w),device=x[3].device)
            for imi, imb in enumerate(img_boxes[0].long()):
                if img_tamps[0][imi] == 1:
                    msk0[:, :, imb[1]:imb[3], imb[0]:imb[2]] = 0
            if msk0.sum()<1:
                flag0 = False
        else:
            flag0 = False
        if ((len(img_boxes[1])!=0) and (len(valid_boxes1)!=0)):
            flag1 = True
            h,w =  x[3].shape[-2:]
            msk1 = torch.ones((1, 1, h, w),device=x[3].device)
            for imi, imb in enumerate(img_boxes[1].long()):
                if img_tamps[1][imi] == 1:
                    msk1[:, :, imb[1]:imb[3], imb[0]:imb[2]] = 0
            if msk1.sum()<1:
                flag1 = False
        else:
            flag1 = False
        ...
        cosl_loss = 0 
        if flag0:
            valid_boxes0 = [torch.stack(valid_boxes0).to(device)]
            mask_rois = mask_target(valid_boxes0, [valid_index0], [batch_data_samples[0].gt_instances.masks], self.same_cfg) # weight
            feat_roi0 = self.msroialign([xx[0:1] for xx in x], valid_boxes0, img_shape)
            feat_roi0 = self.roi_head.txt(feat_roi0[:, 128:]) # self.roi_head.convert2(feat_roi0[:, 128:])
            glob_roi0 = self.roi_head.txt((x[3][0:1,128:]*msk0).sum((2,3), keepdim=True)/msk0.sum())
            cos = torch.abs(self.cos(feat_roi0, glob_roi0.expand_as(feat_roi0)))
            cosl_label = (1-torch.cuda.FloatTensor(valid_tamper0,device=device))
            mask_rois_ = mask_rois.clone()
            # print('mask_rois', mask_rois_.shape, cosl_label.nonzero().squeeze(1))
            mask_rois_[cosl_label.nonzero().squeeze(1)]=1
            cosl_label = cosl_label.unsqueeze(1).unsqueeze(1).expand_as(cos)
            # print('cos0',cos.max(), cos.min(), cosl_label.max(), cosl_label.min(), len(valid_tamper0), cosl_label.shape)
            cosl_loss = (cosl_loss + F.binary_cross_entropy(cos, cosl_label, weight=mask_rois_))
            # print(valid_boxes0, batch_inputs.shape)
            # print(feat_roi0.mean())
            # print(glob_roi0.mean())
            # print('cos', cos.mean())
            # print('coslable', cosl_label.mean())
            # print('coslloss', cosl_loss)
            # print('shape', mask_rois.shape, cos.shape, cosl_label.nonzero().squeeze(1).shape)
            # exit(0)
        if flag1:
            valid_boxes1 = [torch.stack(valid_boxes1).to(device)]
            mask_rois = mask_target(valid_boxes1, [valid_index1], [batch_data_samples[1].gt_instances.masks], self.same_cfg) # weight
            feat_roi1 = self.msroialign([xx[1:2] for xx in x], valid_boxes1, img_shape)
            feat_roi1 = self.roi_head.txt(feat_roi1[:, 128:]) # self.roi_head.convert2(feat_roi0[:, 128:])
            glob_roi1 = self.roi_head.txt((x[3][1:2,128:]*msk1).sum((2,3), keepdim=True)/msk1.sum())
            cos = torch.abs(self.cos(feat_roi1, glob_roi1.expand_as(feat_roi1)))
            cosl_label = (1-torch.cuda.FloatTensor(valid_tamper1,device=device))
            mask_rois_ = mask_rois.clone()
            mask_rois_[cosl_label.nonzero().squeeze(1)]=1
            cosl_label = cosl_label.unsqueeze(1).unsqueeze(1).expand_as(cos)
            cosl_loss = (cosl_loss + F.binary_cross_entropy(cos, cosl_label, weight=mask_rois_))
            # print(valid_boxes1, batch_inputs.shape)
            # print(feat_roi1.mean())
            # print(glob_roi1.mean())
            # print('cos', cos.mean())
            # print('coslable', cosl_label.mean())
            # print('coslloss', cosl_loss)
            # print('shape', mask_rois.shape, cos.shape, cosl_label.nonzero().squeeze(1).shape)
        ...
        if cosl_loss!=0:
            losses = dict(cosl_loss=cosl_loss)
        else:
            losses = dict()
