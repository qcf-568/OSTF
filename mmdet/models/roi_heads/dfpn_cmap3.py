...
class DFPNCMap3(BaseRoIHead):
    ...
    def __init__(self,
        ...
        self.convert = nn.Linear(256, 1024) # ConvModule(256, 1024, 1)
    ...
    def init_mask_head(self, mask_roi_extractor: ConfigType,
        ...
        # self.mask_head2 = MODELS.build(mask_head) # nn.Sequential(ConvModule(256, 256, 3, stride=2, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), ConvModule(256, 256, 3, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), ConvModule(256, 256, 3, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), nn.Flatten(), nn.Linear(256, 2))
    ...
    def loss(self, x1: Tuple[Tensor], x2: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        losses = dict()
        # bbox head loss
        bbox_results, this_inds = self.bbox_loss(x1, sampling_results)
        if ('loss_cls' in bbox_results.keys()):
        ...
        adv_label = None # torch.cat([res.ttypes[pos_assigned_gt_inds[ri]] for ri,res in enumerate(batch_gt_instances)])
        # mask head forward and loss
        if self.with_mask:
            mask_results = self.mask_loss(x2, sampling_results, None, batch_gt_instances, gt_label, adv_label, this_inds)
    ...
    def bbox_loss(self, x: Tuple[Tensor],
        ...
        if ('loss_cls' in bbox_loss_and_target.keys()):
            bbox_results.update(dict(loss_cls=bbox_loss_and_target['loss_cls'], loss_bbox=bbox_loss_and_target['loss_bbox']))
        else:
            print('error')
            bbox_results.update(dict(loss_bbox=bbox_loss_and_target['loss_bbox']))
        return bbox_results, this_inds
    ...
    def mask_loss(self, x: Tuple[Tensor],
            ...
            for res in sampling_results:
                ...
                pos_inds.append(
                    torch.zeros(
                        ...
                        dtype=torch.uint8))
            pos_inds = torch.cat(pos_inds)
            mask_results, glb_feats = self._mask_forward(x, pos_inds=pos_inds, bbox_feats=bbox_feats, this_inds=this_inds)
        ...
        # print('shapes', mask_results.shape, glb_feats.shape, gt_label.shape)
        if glb_feats is None:
            # loss_tamp = self.celoss(self.clsfc(mask_results).float(), gt_label.long())
            # loss_adv = self.celoss(self.advfc(mask_results).float(), adv_label.long())
            loss_cos = None
        else:
            # loss_tamp = self.celoss(self.clsfc(torch.cat((mask_results, glb_feats[0:1]), 0)).float(), torch.cat((gt_label.long(), torch.cuda.LongTensor([0], device=gt_label.device)), 0))
            # loss_adv = self.celoss(self.advfc(mask_results).float(),adv_label.long())
