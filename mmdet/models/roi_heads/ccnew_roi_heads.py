...
class CCNewustomRoIHead(BaseRoIHead):
    ...
    def __init__(self,
        ...
        self.cls_fcfc = nn.Linear(1024, 2)
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        if self.with_mask and (len(tamp_feats)>0):
            mask_results = self.mask_loss(x, sampling_results, tamp_feats, batch_gt_instances, gt_label, this_inds)
    ...
    def mask_loss(self, x: Tuple[Tensor],
        ...
        if not self.share_roi_extractor:
            ...
            mask_results = self._mask_forward(x, pos_inds=pos_inds, bbox_feats=bbox_feats, this_inds=this_inds)
        ...
        if rois is not None:
            ...
            if use_inds.shape[0]!=0:
                ...
                if self.training:
                    ...
                    if (rands<0.3):
                        cls_feat = self.mask_head(mask_feats-gloabl_feats) # torch.cat((mask_feats, mask_feats-gloabl_feats), 1))
                    elif (rands<0.6):
                        cls_feat = self.mask_head(mask_feats)
                    else:
                        cls_feat = (self.mask_head(mask_feats-gloabl_feats) + self.mask_head(mask_feats))/2.0
                else:
                    cls_feat = self.mask_head(mask_feats-gloabl_feats)
            else:
                cls_feat = self.mask_head(mask_feats)
        return self.cls_fcfc(cls_feat)
