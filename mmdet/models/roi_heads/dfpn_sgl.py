...
class Texture(nn.Module):
    ...
    def forward(self, x0):
        ...
        x = F.normalize(self.conv12(x),1)
        return torch.cat((x, x0), 1)


@MODELS.register_module()
class DFPNSGL(BaseRoIHead):
    ...
    def __init__(self,
        ...
        self.sgl = SingleCenterLoss(m = 0.3, D = 256)
        self.gfc = nn.Linear(256, 1024)
        self.xfc = nn.Linear(2048, 1024)
        self.fc = nn.Linear(256, 2) # nn.Sequential(nn.Dropout(p=0.3), nn.Linear(1024, 1024), nn.ReLU(True), nn.Dropout(p=0.3), nn.Linear(1024, 2))
    ...
    def loss(self, x1: Tuple[Tensor], x2: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        if self.with_mask:
            ...
            if not (mask_results is None):
                losses.update(dict(loss_cos=mask_results))
    ...
    def mask_loss(self, x: Tuple[Tensor],
        ...
        if True:#not self.share_roi_extractor:
            ...
            mask_results, glb_feats = self._mask_forward(x, pos_inds=pos_inds, bbox_feats=bbox_feats)
        ...
        if False : #glb_feats is None:
            ...
            if (len(gt_valid)>0):
                mskf = mask_results[gt_valid]
                loss_sgl = (self.sgl(mskf, 1 - gt_label[gt_valid].long())[0] / 64.0)
                loss_cos = (self.celoss(self.fc(mskf), gt_label[gt_valid].long())  + loss_sgl)
            else:
                loss_cos = None
    ...
    def cosall(self, m, g):
        simi = self.cos(m,g)
        return torch.stack((simi, 1-simi), 1)
    ...
    def predict_mask(self,
        ...
        if False:#glb_feats is None:
            ...
            return [F.softmax(self.fc(m),1) for m in mask_feats]
