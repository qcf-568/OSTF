...
class CustomRoIHead(BaseRoIHead):
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        if ('loss_cls' in bbox_results.keys()):
            losses.update(dict(loss_cls=bbox_results['loss_cls']))
        if ('loss_bbox' in bbox_results.keys()):
            losses.update(dict(loss_bbox=bbox_results['loss_bbox']))
    ...
    def bbox_loss(self, x: Tuple[Tensor],
        ...
        if ('loss_cls' in bbox_loss_and_target.keys()):
            if (bbox_loss_and_target['loss_cls']<16):
                bbox_results.update(dict(loss_cls=bbox_loss_and_target['loss_cls']))
        if ('loss_bbox' in bbox_loss_and_target.keys()):
            if (bbox_loss_and_target['loss_bbox']<16):
    ...
    def predict(self,
        ...
        tamp_score = self.predict_mask(x, batch_img_metas, results_list, tamp_feats, rescale=rescale)
    ...
    def predict_mask(self,
        ...
        mask_preds = self._mask_forward(x, pos_inds=inds, bbox_feats=tamp_feats)
        # mask_preds = mask_results['mask_preds']
