...
class FustomRoIHead(BaseRoIHead):
    ...
    def __init__(self,
        ...
        self.texture_cls_head = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten())#ConvModule(256, 256, 3, padding=0, norm_cfg=self.norm_cfg), ConvModule(256, 256, 3, padding=0, norm_cfg=self.norm_cfg), ConvModule(256, 256, 3, padding=0, norm_cfg=self.norm_cfg),nn.Flatten(start_dim=1),nn.Linear(256,2))
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        losses.update(dict(loss_cls=bbox_results['loss_cls'], loss_bbox=bbox_results['loss_bbox'], loss_tamp=bbox_results['loss_tamp']))
        # mask head forward and loss
        if self.with_mask:
            mask_results = self.mask_loss(x, sampling_results,
                                          bbox_results['reg_feats'],
                                          batch_gt_instances)
    ...
    def _bbox_forward(self, x: Tuple[Tensor], rois: Tensor) -> dict:
        ...
        tmp_feats = self.convert2(bbox_feats[:,128:])
        cls_score, bbox_pred = self.bbox_head1(reg_feats)
        # tmp_score, _ = self.tamp_head(tmp_feats)
        bbox_results = dict(cls_score=cls_score, bbox_pred=bbox_pred, reg_feats=reg_feats, tmp_feats=tmp_feats)
        return bbox_results

    def bbox_loss(self, x: Tuple[Tensor],
        sampling_results: List[SamplingResult], batch_gt_instances) -> dict:
        ...
        gt_tamper = [res.tamper for res in batch_gt_instances]
        pos_inds = []
        device = x[0].device
        ...
        pos_tmps = bbox_results['tmp_feats'][pos_inds]
        gt_tamp = torch.cat([res.tamper[pos_assigned_gt_inds[ri]] for ri,res in enumerate(batch_gt_instances)])
        pred_tamp = self.texture_cls_head(pos_tmps)
        ...
        loss_tamp = self.celoss(pred_tamp, gt_tamp)
        bbox_results.update(dict(loss_cls=bbox_loss_and_target['loss_cls'], loss_bbox=bbox_loss_and_target['loss_bbox'], loss_tamp=loss_tamp))
        return bbox_results

    def tamp_loss(self, x: Tuple[Tensor],
                  sampling_results: List[SamplingResult], bbox_feats: Tensor,
                  batch_gt_instances: InstanceList) -> dict:
        if False:#not self.share_roi_extractor:
            pos_rois = bbox2roi([res.pos_priors for res in sampling_results])
            tamp_results = self._tamp_forward(x, pos_rois)
        else:
            pos_inds = []
            ...
            tamp_results = self._tamp_forward(
                x, pos_inds=pos_inds, bbox_feats=bbox_feats)

        tamp_loss_and_target = self.tamp_head.loss_and_target(
            cls_score=bbox_results['cls_score'],
            # bbox_pred=bbox_results['bbox_pred'],
            # domain_pd=bbox_results['domain_pd'],
            rois=rois,
    ...
    def _tamp_forward(self,
        ...
        if False: # rois is not None:
            tamp_feats = self.tamp_roi_extractor(
                x[:self.mask_roi_extractor.num_inputs], rois)
            if self.with_shared_head:
                tamp_feats = self.shared_head(mask_feats)
        else:
            assert bbox_feats is not None
            tamp_feats = bbox_feats[pos_inds]

        tamp_preds, _ = self.tamp_head(tamp_feats)
        tamp_results = dict(tamp_preds=tamp_preds)
        return tamp_results
