...
class C2ustomRoIHead(BaseRoIHead):
    ...
    def init_mask_head(self, mask_roi_extractor: ConfigType,
        ...
        self.mask_head = nn.Sequential(ConvModule(256, 256, 3, stride=2, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), ConvModule(256, 256, 3, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), ConvModule(256, 256, 3, padding=1, norm_cfg=dict(type='SyncBN')), nn.MaxPool2d(2,2), ConvModule(256, 256, 3, padding=1, norm_cfg=dict(type='SyncBN')), nn.AvgPool2d(2,2), nn.Flatten(), nn.Linear(256, 2))
        print('imim')
    ...
    def _mask_forward(self,
        ...
        cls_score = self.mask_head(self.convert2(mask_feats[:,128:]))
        return cls_score
    ...
    def predict(self,
        ...
        tamp_score = self.predict_mask(x, batch_img_metas, results_list, rescale=rescale, scales=scales)
    ...
    def predict_bbox(self,
        ...
        bbox_results, _ = self._bbox_forward(x, rois)
    ...
    def predict_mask(self,
        ...
        mask_preds = self._mask_forward(x, mask_rois)
        # mask_preds = mask_results['mask_preds']
