...
class Texture(nn.Module):
    def __init__(self, in_channel, mid_channel):
        ...
        self.conv11 = ConvModule(in_channel, mid_channel, 1, norm_cfg=dict(type='SyncBN')) # BasicConv2d(in_channel, mid_channel, 1)
        self.conv12 = ConvModule((mid_channel - 1) * (mid_channel)//2, out_channel, 1, norm_cfg=dict(type='SyncBN')) # BasicConv2d((mid_channel - 1) * (mid_channel)//2, out_channel, 1)
    ...
    def forward(self, x0):
        B,C,H,W = x0.shape
        means = x0.mean((2,3),keepdim=True).expand_as(x0)
        stds = x0.std((2,3),keepdim=True).expand_as(x0)
        x = self.conv11(x0)
        ...
        x = self.conv12(x)
        return torch.cat((x, x0, means, stds), 1)

@MODELS.register_module()
class CTMSRoIHead(BaseRoIHead):
