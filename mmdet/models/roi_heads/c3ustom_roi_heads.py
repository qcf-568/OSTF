...
class C3ustomRoIHead(BaseRoIHead):
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        losses = dict()
        # bbox head loss
        bbox_results, tamp_feats = self.bbox_loss(x, sampling_results)
        ...
        if self.with_mask and (len(tamp_feats)>0):
            mask_results = self.mask_loss(x, sampling_results, tamp_feats, batch_gt_instances, gt_label)
            losses.update(dict(loss_tamper=mask_results))
    ...
    def _bbox_forward(self, x: Tuple[Tensor], rois: Tensor) -> dict:
        ...
        tamp_feats = self.convert2(bbox_feats[:,128:])
        ...
        return bbox_results, tamp_feats
    ...
    def bbox_loss(self, x: Tuple[Tensor],
        ...
        bbox_results, tamp_feats = self._bbox_forward(x, rois)
    ...
    def mask_loss(self, x: Tuple[Tensor],
                  ...
                  batch_gt_instances: InstanceList, gt_label) -> dict:
    ...
    def _mask_forward(self,
        ...
        if True:#rois is not None:
            mask_feats = self.convert2(self.bbox_roi_extractor(x[:self.bbox_roi_extractor.num_inputs], rois)[:,128:])
            if self.with_shared_head:
                mask_feats = self.shared_head(mask_feats)
            # print(mask_feats.shape, bbox_feats.shape, (mask_feats-bbox_feats).mean())
        else:
    ...
    def predict(self,
        ...
        results_list, tamp_feats = self.predict_bbox(
            x,
            batch_img_metas,
            rpn_results_list,
            rcnn_test_cfg=self.test_cfg,
            rescale=bbox_rescale)
        # print([rpn_results_list[i][results_list[i].inds].bboxes for i in range(len(rpn_results_list))], [res.bboxes for res in results_list])
        tamp_score = self.predict_mask(x, batch_img_metas, [rpn_results_list[i][results_list[i].inds] for i in range(len(rpn_results_list))], results_list, tamp_feats, scales=scales, rescale=rescale)
    ...
    def predict_mask(self,
        ...
        # bboxes = [res.bboxes for res in rpn_results_list]
        bboxes = [self.scale_box(res.bboxes, scales[ri]) for ri,res in enumerate(results_list)]
        print('box', bboxes, [r.bboxes for r in rpn_results_list])
        ...
        mask_preds = self._mask_forward(x, rois=mask_rois, pos_inds=inds, bbox_feats=tamp_feats[inds])#rois=mask_rois)
        # mask_preds = mask_results['mask_preds']
