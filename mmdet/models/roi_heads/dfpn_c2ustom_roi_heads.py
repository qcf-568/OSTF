...
class DFPNC2ustomRoIHead(BaseRoIHead):
    ...
    def loss(self, x1: Tuple[Tensor], x2: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        losses = dict()
        # bbox head loss
        bbox_results = self.bbox_loss(x1, sampling_results)
        ...
        if self.with_mask:
            mask_results = self.mask_loss(x2, sampling_results, None, batch_gt_instances, gt_label)
    ...
    def predict(self,
        ...
        tamp_score = self.predict_mask(x2, batch_img_metas, results_list, scales=scales, rescale=rescale)
