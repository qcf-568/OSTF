...
class CustomAssignResult(util_mixins.NiceRepr):
    ...
    def __init__(self, num_gts: int, gt_inds: Tensor, max_overlaps: Tensor,
            labels: Tensor, ttypes: Tensor) -> None:
    ...
    def __nice__(self):
        ...
        parts = []
        # print('custom_assign_result.py', self.gt_inds.max(), self.max_overlaps.max(), self.labels.max())
        ...
        if self.labels is None:
            ...
            parts.append(f'ttypes={self.ttypes!r}')
            parts.append(f'sames={self.sames!r}')
        else:
    ...
    def add_gt_(self, gt_labels, gt_ttypes):
        ...
        self.ttypes = torch.cat([gt_ttypes, self.ttypes])
        # self.sames = torch.cat([sames, self.sames])
