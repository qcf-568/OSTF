...
class CustomMaxIoUAssigner(BaseAssigner):
    ...
    def __init__(self,
        ...
        self.match_low_quality = False # match_low_quality
    ...
    def assign(self,
        ...
        gt_ttypes = gt_instances.ttypes
        # gt_sames = gt_instances.sames
        ...
        if assign_on_cpu:
            ...
            gt_ttypes = gt_ttypes.cpu()
            # gt_sames = gt_sames.cpu()
        ...
        assign_result = self.assign_wrt_overlaps(overlaps, gt_labels, gt_ttypes)
        if assign_on_cpu:
    ...
    def assign_wrt_overlaps(self, overlaps: Tensor,
            gt_labels: Tensor, gt_ttypes: Tensor) -> CustomAssignResult:
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            assigned_ttypes = overlaps.new_full((num_bboxes, ), -1, dtype=torch.long) 
            if num_gts == 0:
                # No truth, assign everything to background
                assigned_gt_inds[:] = 0
            return CustomAssignResult(
                num_gts=num_gts,
                gt_inds=assigned_gt_inds,
                max_overlaps=max_overlaps,
                labels=assigned_labels,
                ttypes=assigned_ttypes)
        ...
        # print('gt', gt_labels)
        # print('custom_max_iou_assigner.py', max_overlaps.max())
        # exit(0)
        ...
        # print('custom_max_iou_assigner.py', max_overlaps.max(), pos_inds.nonzero())
        assigned_gt_inds[pos_inds] = argmax_overlaps[pos_inds] + 1 # +1 and -1 avoid conflicts with negative 0, which gt to assign

        assigned_labels = assigned_gt_inds.new_full((num_bboxes, ), -1)
        assigned_ttypes = assigned_gt_inds.new_full((num_bboxes, ), -1)
        pos_inds = torch.nonzero(
            assigned_gt_inds > 0, as_tuple=False).squeeze()
        if pos_inds.numel() > 0:
            assigned_labels[pos_inds] = gt_labels[assigned_gt_inds[pos_inds] - 1]
            assigned_ttypes[pos_inds] = gt_ttypes[assigned_gt_inds[pos_inds] - 1]
        # print('custom_max_iou_assigner.py', assigned_labels.max(), assigned_ttypes.max(), CustomAssignResult(num_gts=num_gts, gt_inds=assigned_gt_inds, max_overlaps=max_overlaps, labels=assigned_labels, ttypes=assigned_ttypes))
        return CustomAssignResult(
            num_gts=num_gts,
            gt_inds=assigned_gt_inds,
            max_overlaps=max_overlaps,
