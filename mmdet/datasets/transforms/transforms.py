...
class TextureSG2(BaseTransform):
    def __init__(self, prob=1.0, bathres=2048, revjpegpath='', diffstepath='', q1=75, q2=100, rgbshift=20) -> None:
        assert 0 <= prob <= 1
        self.q1 = q1
        self.q2 = q2
        self.p = prob
       ...
       if (rand<0.5):
           if exists_revjpeg:
               if random.uniform(0,1)<0.33:
                   return (self.revjpeg(xmin, ymin, xmax, ymax, results), 1)
               elif (rands<0.66):
                   return (self.blur(img, bh), 1)
               else:
                   return (self.blur(self.revjpeg(xmin, ymin, xmax, ymax, results), bh), 1)
           else:
               return (self.blur(img, bh), 1)
       else:
           if exists_diffste:
               if random.uniform(0,1)<0.33:
                    return (self.diffste(xmin, ymin, xmax, ymax, results), 1)
               elif (rands<0.66):
                   return (self.blur(img, bh), 1)
               else:
                    return (self.blur(self.diffste(xmin, ymin, xmax, ymax, results), bh), 1)
           elif exists_revjpeg:
               if random.uniform(0,1)<0.33:
                   return (self.revjpeg(xmin, ymin, xmax, ymax, results), 1)
               elif (rands<0.66):
    ...
    def revjpeg(self, xmin, ymin, xmax, ymax, results):
        rst = cv2.imread(os.path.join(self.revjpegpath, results['img_path']))[xmin: xmax, ymin: ymax]
        if random.uniform(0,1)<0.3:
            return self.revblur(rst)
        else:
            return rst

    def diffste(self, xmin, ymin, xmax, ymax, results):
        rst = cv2.imread(os.path.join(self.diffstepath, results['img_path']))[xmin: xmax, ymin: ymax]
        if random.uniform(0,1)<0.3:
            return self.revblur(rst)
        else:
            return rst
    ...
    def blur(self, img, bh):
        rand = random.uniform(0,1)
        if rand<0.3:
            return self.gauss_blur(img, bh*2)
        elif rand<0.6:
            return self.pixelate(img, bh*2)
        elif rand<0.9:
            return self.defocus_blur(img, bh*2)
        else:
            return self.motion_blur(img, bh*2)
...
@TRANSFORMS.register_module()
class TextureSG(BaseTransform):
    def __init__(self, prob=1.0, bathres=2048, revjpegpath='', q1=75, q2=100, rgbshift=20, mins=32) -> None:
        assert 0 <= prob <= 1
        self.q1 = q1
        self.q2 = q2
        self.p = prob
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['gt_bboxes_tamper_type0'] = np.full((len(results['gt_bboxes_labels']),),-1,dtype=np.int64)
        ...
        for bnum in range(lens):#enumerate(results['gt_bboxes']):
            ...
            box = results['gt_bboxes'][bi]
            bh, bw, ba = results['gt_bboxes_whs'][bi].astype(np.int32)
            if ((bh<self.mins) or (bw<self.mins) or (whs[bi][2]<=self.bathres) or nospe):
                continue
            box = box.numpy().squeeze().astype(np.int32)
            y1,x1,y2,x2 = box
            if ((y2>w) or (x2>h) or (y1>=y2) or (x1>=x2)):
        ...
        results['gt_bboxes_labels'] = results['gt_bboxes_tamper_type0']
        # print('gt', lens, results['gt_bboxes_labels'])
        results['img'] = img1 # np.concatenate((img1, img2), 2)
        assert results['img'].shape[2]==3
        return results
    ...
    def img_tamper(self, img, bh, xmin, ymin, xmax, ymax, results):
       rand = random.uniform(0,1)
       # if (rand<0.25):
       #     return (self.jpeg(img, bh), 0)
       if True: # (rand<0.5):
           if os.path.exists(os.path.join(self.revjpegpath, results['img_path'])):
               ...
               print('empty', os.path.join(self.revjpegpath, results['img_path']))
               return (self.blur(img, bh), 1)
       else:
           return (self.blur(img, bh), 1)
    ...
    def revjpeg(self, xmin, ymin, xmax, ymax, results):
        # print(os.path.exists(os.path.join(self.revjpegpath, results['img_path'])), results['img_path'], self.revjpegpath)
...
@TRANSFORMS.register_module()
class Texture(BaseTransform):
    def __init__(self, prob=1.0, bathres=2048, revjpegpath='', q1=75, q2=100, rgbshift=20) -> None:
        assert 0 <= prob <= 1
        self.q1 = q1
        self.q2 = q2
        self.p = prob
    ...
    def transform(self, results: dict) -> dict:
        ...
        img1 = F.shift_rgb(img1, random.randint(-self.rgbshift, self.rgbshift), random.randint(-self.rgbshift, self.rgbshift), random.randint(-self.rgbshift, self.rgbshift))
        img2 = F.shift_rgb(img2, random.randint(-self.rgbshift, self.rgbshift), random.randint(-self.rgbshift, self.rgbshift), random.randint(-self.rgbshift, self.rgbshift))
        img1 = cv2.imdecode(cv2.imencode('.jpg',img1,[1,random.randint(self.q1,self.q2)])[1],1)
        img2 = cv2.imdecode(cv2.imencode('.jpg',img2,[1,random.randint(self.q1,self.q2)])[1],1)
...
class RangeResize(MMCV_Resize):
    ...
    def transform(self, results: dict) -> dict:
        ...
        assert self.scale
        results['scale'] = self.random_sample(self.scale)
        self._resize_img(results)
        # print('rsz',img_shape,results['img'].shape[:2])
...
class RandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            # results['gt_bboxes_labels1'] = results['gt_bboxes_labels1'][valid_inds]
            if 'gt_bboxes_tamper_type1' in results.keys():
                results['gt_bboxes_tamper_type0'] = results['gt_bboxes_tamper_type0'][valid_inds]
                results['gt_bboxes_tamper_type1'] = results['gt_bboxes_tamper_type1'][valid_inds]
                results['gt_bboxes_tamper_same'] = results['gt_bboxes_tamper_same'][valid_inds]
...
@TRANSFORMS.register_module()
class CS(BaseTransform):
    def __init__(self, q1=70, q2=100, s=20) -> None:
        self.q1 = q1
        self.q2 = q2
        self.rgbshift = s
