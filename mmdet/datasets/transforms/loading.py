...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_bboxes(self, results: dict) -> None:
        ...
        for instance in results.get('instances', []):
            ...
            gt_ignore_flags.append(0)#(instance['ignore_flag'])
    ...
    def _load_labels(self, results: dict) -> None:
        ...
        gt_bboxes_labels = []
        for instance in results.get('instances', []):
            if 'bbox_whs' in instance.keys():
                flag = True
            else:
                flag = False
            gt_bboxes_labels.append(instance['bbox_label'])
            if flag:
                gt_bboxes_whs.append(instance['bbox_whs'])
                gt_bboxes_pts.append(instance['bbox_pts'])
        # TODO: Inconsistent with mmcv, consider how to deal with it later.
        results['gt_bboxes_labels'] = np.array(gt_bboxes_labels, dtype=np.int64)
        results['gt_bboxes_whs'] = np.array(gt_bboxes_whs)
        results['gt_bboxes_pts'] = tuple(gt_bboxes_pts)
    ...
    def _load_masks(self, results: dict) -> None:
        ...
        """
        h, w = results['ori_shape']
        gt_masks = cv2.imread(results['msk_path'], 0)
        gt_max = gt_masks.max()
        gt_eye = np.eye(gt_max+1).astype(np.uint8)
        results['gt_masks'] = BitmapMasks(gt_eye[gt_masks].transpose(2,0,1)[1:], h, w)
