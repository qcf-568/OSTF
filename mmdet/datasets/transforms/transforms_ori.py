...
class Texture(BaseTransform):
    def __init__(self, prob: float = 1.0, bathres=2568, revjpegpath='') -> None:
        assert 0 <= prob <= 1
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['gt_bboxes_labels0'] = np.zeros((len(results['gt_bboxes_labels']),),dtype=np.int32)
        results['gt_bboxes_labels1'] = np.zeros((len(results['gt_bboxes_labels']),),dtype=np.int32)
        results['gt_bboxes_tamper_same'] = np.zeros((len(results['gt_bboxes_labels']),),dtype=np.int32)
        tamps=0
        for bi,box in enumerate(results['gt_bboxes']):
            if tamps>32:
                continue
            # hok = (results['gt_bboxes_whs'][bi][0]>=32)
            # wok = (results['gt_bboxes_whs'][bi][1]>=32)
            box = box.numpy().squeeze().astype(np.int32)
            y1,x1,y2,x2 = box
            if ((y1>=y2) or (x1>=x2)):
                continue
            bh, bw, ba = results['gt_bboxes_whs'][bi].astype(np.int32)
            if ((bh<32) or (bw<32) or (ba<2048)):
                continue
            bws = bw//2
            bhs = bh//2
            ...
            if (random.uniform(0,1)<0.5):
                ...
                results['gt_bboxes_labels0'][bi] = 1
            ...
            if (random.uniform(0,1)<0.5):
                ...
                tamps = (tamps+1)
                if flag:
                    results['gt_bboxes_tamper_same'][bi] = 2
            else:
                if (not flag):
                    results['gt_bboxes_tamper_same'][bi] = 1
    ...
    def img_tamper(self, img, bh, xmin, ymin, xmax, ymax, results):
       ...
       if True: # (rand<0.5):
           if os.path.exists(os.path.join(self.revjpegpath, results['img_path'])):
               return (self.revjpeg(xmin, ymin, xmax, ymax, results), 0)
           else:
               print('empty', os.path.join(self.revjpegpath, results['img_path']))
               return (self.blur(img, bh), 0)
       else:
           return (self.blur(img, bh), 1)
       # else:
       #     return (self.revblur(img), 3)
    ...
    # @njit()
    # def _shuffle_pixels_njit_glass_blur(self,d0,d1,x,c):
    ...
    # def glass_blur(self, x, h):
    #     if h<=8:
    #         c = (random.uniform(0.4, 0.6), 1, 2)
    #     elif h<=16:
    #         c = (random.uniform(0.6, 0.8), 1, 2)
    #     elif h<=32:
    #         c = (random.uniform(0.8, 1.0), 1, 2)
    #     elif h<=64:
    #         c = (random.uniform(0.8, 1.0), 2, 2)
    #    elif h<=128:
    #        c = (random.uniform(1.0, 1.25), 2, 2)
    #     else:
    #         c = (random.uniform(1.25, 1.5), 2, 2)
    #     x = np.uint8(gaussian(x / 255., sigma=c[0], multichannel=True) * 255)
    #     x = self._shuffle_pixels_njit_glass_blur(np.array(x).shape[0],np.array(x).shape[1],x,c)
