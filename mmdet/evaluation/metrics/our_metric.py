...
class IC15(BaseMetric):
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]):
        ...
        for ds in data_samples:
            ...
            pd_real = []
            pd_fake = []
            for bi,bs in zip(ds['gt_instances']['labels'].cpu().numpy(), ds['gt_instances']['bboxes'].cpu().numpy()):
                if (bi==0):
                    gt_real.append(bs)
                elif (bi==1):
                    gt_fake.append(bs)
            for bi,bs in zip(ds['pred_instances']['labels'].cpu().numpy(), ds['pred_instances']['bboxes'].cpu().numpy()):
                if (bi==0):
                    pd_real.append(bs)
                elif (bi==1):
                    pd_fake.append(bs)
